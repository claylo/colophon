...
2. Create an initial formula `Formula/colophon.rb`:
...
class Colophon < Formula
  ...
  on_macos do
    if Hardware::CPU.arm?
      url "https://github.com/claylo/colophon/releases/download/v#{version}/colophon-#{version}-aarch64-apple-darwin.tar.gz"
      sha256 "PLACEHOLDER"
    else
      url "https://github.com/claylo/colophon/releases/download/v#{version}/colophon-#{version}-x86_64-apple-darwin.tar.gz"
      sha256 "PLACEHOLDER"
    end
  end
  ...
  on_linux do
    if Hardware::CPU.arm?
      url "https://github.com/claylo/colophon/releases/download/v#{version}/colophon-#{version}-aarch64-unknown-linux-gnu.tar.gz"
      sha256 "PLACEHOLDER"
    else
      url "https://github.com/claylo/colophon/releases/download/v#{version}/colophon-#{version}-x86_64-unknown-linux-gnu.tar.gz"
      sha256 "PLACEHOLDER"
    end
  end

  def install
    bin.install "bin/colophon"
    man1.install Dir["share/man/man1/*.1"]
    bash_completion.install "share/completions/colophon.bash" => "colophon"
    zsh_completion.install "share/completions/_colophon"
    fish_completion.install "share/completions/colophon.fish"
  end
...
assets = [
    # Binary
    ["target/release/colophon", "usr/bin/", "755"],
    # Man pages (if using xtask)
    ["target/dist/share/man/man1/*", "usr/share/man/man1/", "644"],
    # Shell completions
    ["target/dist/share/completions/colophon.bash", "usr/share/bash-completion/completions/colophon", "644"],
    ["target/dist/share/completions/_colophon", "usr/share/zsh/vendor-completions/", "644"],
    ["target/dist/share/completions/colophon.fish", "usr/share/fish/vendor_completions.d/", "644"],
]
```
...
assets = [
    # Binary
    { source = "target/release/colophon", dest = "/usr/bin/colophon", mode = "755" },
    # Man pages (if using xtask)
    { source = "target/dist/share/man/man1/*", dest = "/usr/share/man/man1/", mode = "644", doc = true },
    # Shell completions
    { source = "target/dist/share/completions/colophon.bash", dest = "/usr/share/bash-completion/completions/colophon", mode = "644" },
    { source = "target/dist/share/completions/_colophon", dest = "/usr/share/zsh/site-functions/_colophon", mode = "644" },
    { source = "target/dist/share/completions/colophon.fish", dest = "/usr/share/fish/vendor_completions.d/colophon.fish", mode = "644" },
]
...
```
npm/
├── colophon/                    # Main wrapper package
│   ├── package.json
│   ├── index.js                           # Binary resolution
│   ├── cli.js                             # CLI entry point
│   └── install.js                         # Postinstall fallback
└── platforms/
    ├── colophon-darwin-arm64/
    │   ├── package.json
    │   └── bin/colophon
    ├── colophon-darwin-x64/
    ├── colophon-linux-arm64/
    ├── colophon-linux-x64/
    ├── colophon-win32-arm64/
    └── colophon-win32-x64/
```

#### Platform Package (e.g., `npm/platforms/colophon-linux-x64/package.json`)
...
```json
{
  ...
  "repository": {
    "type": "git",
    "url": "https://github.com/claylo/colophon"
  },
  "os": ["linux"],
  "cpu": ["x64"],
  "files": ["bin/"]
...
#### Main Wrapper Package (`npm/colophon/package.json`)
...
```json
{
  ...
  "optionalDependencies": {
    "@claylo/colophon-darwin-arm64": "0.1.0",
    "@claylo/colophon-darwin-x64": "0.1.0",
    "@claylo/colophon-linux-arm64": "0.1.0",
    "@claylo/colophon-linux-x64": "0.1.0",
    "@claylo/colophon-win32-arm64": "0.1.0",
    "@claylo/colophon-win32-x64": "0.1.0"
  }
}
```

#### Binary Resolution (`npm/colophon/index.js`)
...
function getBinaryPath() {
  ...
  const binaryName = process.platform === "win32" ? "colophon.exe" : "colophon";
...
#### CLI Entry Point (`npm/colophon/cli.js`)
...
#### Postinstall Fallback (`npm/colophon/install.js`)
...
```
colophon-{version}-{target}/
├── bin/
│   └── colophon          # The compiled binary
├── share/
│   ├── man/
│   │   └── man1/
│   │       ├── colophon.1           # Main command man page
│   │       └── colophon-*.1         # Subcommand man pages
│   └── completions/
│       ├── colophon.bash            # Bash completions
│       ├── colophon.fish            # Fish completions
│       ├── colophon.ps1             # PowerShell completions
│       └── _colophon                # Zsh completions
├── LICENSE-*
├── README.md
└── CHANGELOG.md
```
...
```bash
cargo audit bin ./target/release/colophon
```
