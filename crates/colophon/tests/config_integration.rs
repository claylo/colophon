...
fn discovers_dotfile_config_in_current_dir() {
    let tmp = TempDir::new().unwrap();
    let config_path = tmp.path().join(".colophon.toml");
    ...
    assert!(
        reported.ends_with(".colophon.toml"),
...
fn discovers_config_in_parent_directory() {
    ...
    fs::write(tmp.path().join(".colophon.toml"), r#"log_level = "debug""#).unwrap();
...
fn discovers_dotconfig_directory_config() {
    ...
    fs::write(dotconfig.join("colophon.toml"), r#"log_level = "debug""#).unwrap();
...
fn dotconfig_takes_precedence_over_dotfile() {
    ...
    fs::write(dotconfig.join("colophon.toml"), r#"log_level = "debug""#).unwrap();
    fs::write(tmp.path().join(".colophon.toml"), r#"log_level = "error""#).unwrap();
...
fn parses_toml_config() {
    let tmp = TempDir::new().unwrap();
    fs::write(tmp.path().join(".colophon.toml"), r#"log_level = "warn""#).unwrap();
...
fn parses_yaml_config() {
    let tmp = TempDir::new().unwrap();
    fs::write(tmp.path().join(".colophon.yaml"), "log_level: warn\n").unwrap();
...
fn parses_yml_config() {
    let tmp = TempDir::new().unwrap();
    fs::write(tmp.path().join(".colophon.yml"), "log_level: debug\n").unwrap();
...
fn closer_config_takes_precedence() {
    ...
    fs::write(tmp.path().join(".colophon.toml"), r#"log_level = "error""#).unwrap();
    fs::write(sub_dir.join(".colophon.toml"), r#"log_level = "debug""#).unwrap();
...
fn toml_preferred_over_yaml_in_same_directory() {
    ...
    fs::write(tmp.path().join(".colophon.toml"), r#"log_level = "debug""#).unwrap();
    fs::write(tmp.path().join(".colophon.yaml"), "log_level: error\n").unwrap();
...
fn invalid_json_config_shows_error() {
    let tmp = TempDir::new().unwrap();
    fs::write(tmp.path().join(".colophon.json"), "{not valid json}").unwrap();
...
fn git_boundary_stops_config_search() {
    ...
    fs::write(parent.join(".colophon.toml"), r#"log_level = "error""#).unwrap();
...
fn config_in_same_dir_as_git_is_found() {
    ...
    fs::create_dir(repo.join(".git")).unwrap();
    fs::write(repo.join(".colophon.toml"), r#"log_level = "debug""#).unwrap();
